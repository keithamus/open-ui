...
when clicked, touched, or activated via keypress - dispatch a `CommandEvent`
...
button that was activated causing the `CommandEvent` to fire. The dispatched
...
| `<dialog>`            | `'toggle-modal'`       | If the `<dialog>` is `open`, then close it, otherwise open it as modal.                                         |
